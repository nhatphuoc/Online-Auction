...
type Product struct {
	...
	CurrentBidCount int        `json:"current_bid_count" pg:"current_bid_count"` // Số lượt ra giá
	EndAt           time.Time  `json:"end_at" pg:"end_at"`
	CreatedAt       time.Time  `json:"created_at" pg:"created_at"`
}

// BidderInfo represents information about the current highest bidder
...
type ProductESDocument struct {
	...
	CurrentBidCount     int         `json:"current_bid_count"` // Số lượt ra giá
	EndAt               time.Time   `json:"end_at"`
	CreatedAt           time.Time   `json:"created_at"`
	
	// Thông tin bidder hiện tại (denormalized for faster search)
	CurrentBidderInfo   *BidderInfo `json:"current_bidder_info,omitempty"`
}

// ProductListingResponse represents product information for listing page
type ProductListingResponse struct {
	...
	CreatedAt         time.Time   `json:"created_at"`
	EndAt             time.Time   `json:"end_at"`
	TimeRemaining     string      `json:"time_remaining"`     // Thời gian còn lại (human-readable)
	CurrentBidCount   int         `json:"current_bid_count"`  // Số lượt ra giá
	Status            string      `json:"status"`
	CategoryName      string      `json:"category_name"`
}
...
type SearchResponse struct {
	Products   []ProductListingResponse `json:"products"`
